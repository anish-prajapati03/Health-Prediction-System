...
path=os.path.dirname(__file__) #provide root path of my app
model=joblib.load(open(os.path.join(path,'best_model.pkl'),'rb'))
label_encode=joblib.load(open(os.path.join(path, 'label_encoder.pkl'), 'rb'))
...
def prediction(req):
    if req.method=='POST':
        ...
        res=label_encode.inverse_transform([result])[0]
